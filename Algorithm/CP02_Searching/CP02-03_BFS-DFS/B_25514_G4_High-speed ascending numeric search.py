...
def solution(board, sr, sc):
    ...
    for tr, tc in target_rc:
        if get_move_cnt(board, sr, sc, tr, tc) == -1:
            return -1 
        else:
            ans += get_move_cnt(board, sr, sc, tr, tc)
            sr, sc = tr, tc
...
# code ref: joonlab(1542) / backjoon(25514) / 알고리즘 분류: 그래프이론, 그래프탐색, 너비우선탐색